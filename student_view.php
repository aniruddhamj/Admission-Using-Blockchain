<?php 
	...
	if(isset($_SESSION['userdata']) && !empty($_SESSION['userdata']) && $_SESSION['userdata']->role == "admin4")
	{
		$get = $_GET;
		if(isset($get['aid']))
		{
...
?>
			<main class="app-content">
				...
				<div class="row">
					<div class="col-md-12">
						<form action="submit_verification.php" method="post" id="admissionForm">
							...
							<div class="tile">
								<h4>Basic Information<a style="float: right;"><i class="fa fa-check text-success"></i></a></h4>
								<hr>
								<div class="row">
									<div class="col-lg-4">
				...
				<div class="tile">
					<h4>Document Information<a style="float: right;"><i class="fa fa-check text-success"></i></a></h4><hr>
					<?php
					...
					<div class="row">
						...
						?>	
										<div class="col-lg-4">
											<center>
												<img src="assets/uploads/<?php echo $val->document_file; ?>" class="img img-responsive img-thumbnail" style="width: 200px;height: 150px;" /><br>
												<?php echo $val->document_name; ?><br>
												<a href="assets/uploads/<?php echo $val->document_file; ?>" download>Download</a>
											</center>
										</div>
						<?php
									}
				...
				<div class="tile">
					<h4>Payment Information<a style="float: right;"><i class="fa fa-check text-success"></i></a></h4><hr>
					<?php
					    $order_id = "-";
					    $order_status = "<span class='badge badge-warning'>Payment Pending</span>";
					    $order_date_time = "-";
						...
						if($payments)
                        {
                            ...
                            {
                                if($payment->admission_id == $admission->id && $payment->payment_status == 1)
                                {
                                    $order_id = $payment->payment_order_id;
            					    $order_status = "<span class='badge badge-success'>Payment Done</span>";
            					    $order_date_time = date("d M, Y",strtotime($payment->created_at));
                                }
                            }
                        }
					?>
					<div class="row">
					    <div class="col-lg-12">
    					    <table width="100%">
    					        <tbody>
    					            <tr>
    					                <td width="20%">Payment Order ID</td>
    					                <td width="80%">: <?php echo $order_id; ?></td>
    					            </tr>
    					            <tr>
    					            ...
    					            <tr>
    					                ...
    					                <td>: <?php echo $order_date_time; ?></td>
    					            </tr>
    					        </tbody>
    					    </table>
    				    </div>
				...
				<?php
					if($admission->basic_verification == 1 && $admission->document_verification == 1 && $admission->payment_verification == 1 && $admission->final_verification == 0)
					{
				?>
						<div class="tile">
							<a class="btn btn-warning" href="submit_verification.php?aid=<?php echo $get['aid']; ?>&payment=received">Verify All Information</a>
						</div>
				<?php 
					}
				?>
